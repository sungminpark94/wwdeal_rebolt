...
const Profile = () => {
  ...
  // Firebase 인증 상태 확인 수정
  useEffect(() => {
    const auth = getAuth();
    
    console.log('Profile - 현재 인증 상태 확인 중...');
    
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      console.log('Profile - 인증 상태 변경:', currentUser);
      setAuthChecked(true);
      
      // 인증되지 않은 사용자는 로그인 모달을 표시하지 않고 로딩만 종료
      if (!currentUser) {
        console.log('인증된 사용자 없음');
        setLoading(false); // 로딩 상태 종료
        // 모달은 표시하지 않음 (setIsLoginModalOpen(true) 제거)
      }
    });
  ...
  useEffect(() => {
    ...
    console.log('- 인증 확인 상태:', authChecked);
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      if (!user) {
        console.log('사용자 없음 - 예약 정보 불러오지 않음');
        setLoading(false);
        return;
      }

      console.log('예약 정보 불러오기 시작, 사용자 ID:', user.uid);
      try {
        const q = query(
        ...
        console.log('예약 정보 불러오기 성공:', reservationList.length, '건');
        setVisitReservations(reservationList);
      } catch (error) {
        console.error('예약 정보 조회 중 오류:', error);
      } finally {
        setLoading(false); // 성공하든 실패하든 로딩 상태 종료
      }
    };

    // 인증이 완료된 상태에서만 예약 정보 조회
    if (user && authChecked) {
      console.log('사용자 정보 있음, 예약 정보 조회 시작');
      fetchReservations();
    } else if (authChecked) {
      // 인증은 확인됐지만 사용자가 없는 경우 로딩 종료
      setLoading(false);
    }
  }, [user, authChecked]); // authChecked 의존성 추가

  // 사용자 정보 불러오기 개선
  useEffect(() => {
    const fetchUserData = async () => {
      if (!user?.uid) {
        console.log('사용자 UID 없음 - 사용자 정보 불러오지 않음');
        return;
      }

      console.log('사용자 정보 불러오기 시작, UID:', user.uid);
      try {
        ...
        {user ? (
          <>
            ...
            <button 
              ...
              className="w-full px-4 py-4 text-left text-gray-600 hover:bg-gray-50 flex justify-between items-center bg-white rounded-xl"
            >
              <span>매물 등록</span>
              <span className="material-icons text-gray-400">chevron_right</span>
            </button>
